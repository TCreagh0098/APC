...
def choice_square_or_rectangle(shape):
    length = num_check("Enter the length: ", "Please enter a valid positive number (e.g., 4): ", float)
    width = num_check("Enter the width: ", "Please enter a valid positive number (e.g., 4): ", float)
...
def choice_oval():
    ...
    if radius_horizontal == "?" or radius_vertical == "?":
        return "?", "?"
...
calculation_history_list = []  # Initialize an empty list
...
Select shape: ''')
    ...
    # Create calculation history dataframe if it doesn't exist
    if 'calculation_history' not in locals():
        calculation_history = pd.DataFrame(columns=["Shape", "Perimeter", "Area"])

    # Append calculation result to the history dataframe
    calculation_history = pd.concat([
        calculation_history,
        pd.DataFrame({"Shape": [shape_choice.capitalize()], "Perimeter": [perimeter], "Area": [area]})
    ], ignore_index=True)
    ...
    # asks user if they wish to continue with the program
    if yes_no("\nDo you wish to find another area and perimeter? (y/n): ") == "no":
